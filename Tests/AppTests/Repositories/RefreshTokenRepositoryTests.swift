...
final class RefreshTokenRepositoryTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        repository = DatabaseRefreshTokenRepository(database: app.db(.sqlite))
    ...
    func testCreatingToken() throws {
        ...
        let token = try RefreshToken(userID: user.requireID(), token: "123")
    ...
    func testDeleteFor() throws {
        try user.create(on: app.db).wait()
        let token = RefreshToken(userID: try user.requireID(), token: "123")
        try token.create(on: app.db).wait()
        try repository.delete(for: token.$user.id).wait()
        let count = try RefreshToken.query(on: app.db).count().wait()
        XCTAssertEqual(count, 0)
    }
    
    func testGetter() throws {
        let count = try app.repositories.refreshTokens.count().wait()
        XCTAssertEqual(count, 0)
    }
}
